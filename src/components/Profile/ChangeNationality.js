...
export default class ChangeNationality extends Component {
    constructor(props) {
        super(props);
        this.state = {
            infoAttribute: "nationality",
            allInfo: [],
            infoContent: '',
        }
    ...
    handleProfileChange(event) {        
        ...
        db.ref('/users/' + uid + '/profile/').update({ // set: Writes data to this Database location. This will overwrite any data at this location and all child locations.
            nationality: this.state.infoContent,
        });
        ...
        */
        alert("Successfully changed nationality!")
    }
    handleChange(event) {
        const uid = auth().currentUser.uid;
    ...
    componentWillReceiveProps(props) {
        ...
        db.ref('/users/' + uid + '/personInfo/nationality').on('value', snapshot => { // ref.on is for reading data from a database
            let allInfo = [];
